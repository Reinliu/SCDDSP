...
class ScriptDDSP(torch.nn.Module):
    ...
    def forward(self, spec_centroid, loudness, mh_distances):
        ...
        print(self.realtime)
        if self.realtime:
            spec_centroid = spec_centroid[:, ::self.ddsp.block_size]
            loudness = loudness[:, ::self.ddsp.block_size]
            mh_distances = mh_distances[:, ::self.ddsp.block_size]
            return self.ddsp.realtime_forward(spec_centroid, loudness)
        else:
            #z, mu, logvar = self.ddsp.encoder(signal)
            #z = torch.zeros(pitch.shape)
            signal, noise, transient = self.ddsp(spec_centroid, loudness, mh_distances)
            return signal
