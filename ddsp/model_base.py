...
class Decoder(nn.Module):
    def __init__(self, hidden_size, n_harmonic, n_bands, sampling_rate, block_size):
        ...
        self.in_mlps = nn.ModuleList([
            ...
            mlp(7, hidden_size, 3)   # For z with dim 1
        ])
        self.gru = gru(3, hidden_size)
        #self.out_mlp = mlp(hidden_size + 19, hidden_size, 3) # when z space is 16
        self.out_mlp = mlp(hidden_size + 10, hidden_size, 3) # when z space is 1
    ...
    def forward(self, pitch, confidence, loudness, onset, mh_distances):
        ...
        hidden = torch.cat([self.gru(hidden)[0], pitch, loudness, onset, mh_distances], -1)
