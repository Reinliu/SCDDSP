...
class Decoder(nn.Module):
    ...
    def forward(self, spec_centroid, loudness, gamma, beta):
        hidden = torch.cat([
    ...
    def realtime_forward(self, spec_centroid, loudness, gamma, beta):
        hidden = torch.cat([
            self.in_mlps[0](spec_centroid),
            self.in_mlps[1](loudness),
        ], -1)
...
class DDSP(nn.Module):
    def __init__(self, hidden_size, n_bands, n_classes, sampling_rate, block_size):
        super(DDSP, self).__init__()
        self.mh_extractor = mh_extractor(n_classes, hidden_size)
        self.decoder = Decoder(hidden_size, n_bands, sampling_rate, block_size)
        
    def forward(self, spec_centroid, loudness, mh_distances):
        gamma, beta = self.mh_extractor(mh_distances)
        signal, noise, transient = self.decoder(spec_centroid, loudness, gamma, beta)
        return signal, noise, transient

    def freeze_decoder(self, training = True):
        for param in self.decoder.parameters():
            param.requires_grad = training

    def unfreeze_mh(self, training = True):
        for param in self.mh_extractor.parameters():
            param.requires_grad = training
