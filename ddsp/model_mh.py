...
class DDSP(nn.Module):
    def __init__(self, hidden_size, n_bands, n_classes, sampling_rate,
        ...
        self.gru = gru(1, hidden_size)  
        self.out_mlp = mlp(hidden_size + 1, hidden_size, 3)
        ...
        self.gamma_layer = nn.Linear(n_classes, hidden_size)
        self.beta_layer = nn.Linear(n_classes, hidden_size)

    def forward(self, loudness, mh_distances):
        hidden = self.in_mlps[0](loudness)
        ...
        hidden = torch.cat([self.gru(hidden)[0], loudness], -1)
