...
import yaml
from ddsp.model_mh import DDSP
...
for e in tqdm(range(epochs)):
    for s, l, m, o in dataloader:
        ...
        signal, noise, transient = model(l, m)
        ...
        onset = onset*s
        transient_loss = (transient-onset).abs().mean()
        ...
        # ori_stft_t = multiscale_fft(
        #     onset,
        #     config["train"]["scales"],
        #     config["train"]["overlap"],
        # )
        # rec_stft_t = multiscale_fft(
        #     transient,
        #     config["train"]["scales"],
        #     config["train"]["overlap"],
        # )
        ...
        # t_loss = 0
        # # Linear, Log Loss
        # for s_x, s_y in zip(ori_stft_t, rec_stft_t):
        #     # lin_loss = (s_x - s_y).abs().mean()
        #     log_loss_t = (safe_log(s_x) - safe_log(s_y)).abs().mean()
        #     t_loss = t_loss + log_loss_t
        # loss+=t_loss
    ...
    if not e % 1: 
        print('loss', loss, 'noise mean', noise.mean(), 'transient loss', transient_loss)